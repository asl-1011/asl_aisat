...
export default function AdminDashboard() {
    ...
    const handleSaveTeam = async () => {
        ...
        try {
            const url = selectedTeam ? `/api/admin/teams/${selectedTeam._id}` : "/api/admin/teams";
            ...
            setTeams(selectedTeam ? teams.map(t => (t._id === selectedTeam._id ? data : t)) : [...teams, data]);
                    ...
                    {teams.map((team) => (
                        <motion.div key={team._id} className="p-5 bg-white/20 backdrop-blur-lg border border-white/30 rounded-xl shadow-lg">
                            <div className="flex items-center gap-4">
                                ...
                                <button onClick={() => openEditModal(team)} className="text-blue-300 hover:text-blue-500"><Edit size={18} /></button>
                ...
                {showEditModal && (
                    <motion.div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4">
                        <motion.div className="relative bg-white text-black p-6 rounded-xl shadow-xl w-full max-w-lg">
                            ...
                            <input type="text" placeholder="Team Name" value={newTeam.team_name} 
                                onChange={(e) => setNewTeam({ ...newTeam, team_name: e.target.value })} className="mt-3 w-full p-2 border rounded-md" />

                            <input type="text" placeholder="Manager Name" value={newTeam.manager} 
                                onChange={(e) => setNewTeam({ ...newTeam, manager: e.target.value })} className="mt-3 w-full p-2 border rounded-md" />

                            <input type="file" accept="image/*" 
                                onChange={(e) => setTeamLogo(e.target.files[0])} className="mt-3 w-full p-2 border rounded-md" />
